package com.example.paymentservice.exception.exceptionHandler;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> webhookFail(WebhookVerificationException ex, WebRequest req) {
        log.warn("Webhook verification failed: {}", ex.getMessage());
        return build(HttpStatus.BAD_REQUEST, "Webhook Verification Failed", ex.getMessage(), req, null);
    }

    @ExceptionHandler(PaymentProviderException.class)
    public ResponseEntity<ErrorResponse> providerErr(PaymentProviderException ex, WebRequest req) {
        log.error("Payment provider error", ex);
        return build(HttpStatus.BAD_GATEWAY, "Payment Provider Error",
                "Payment provider error: " + ex.getMessage(), req, null);
    }
    ...
    public ResponseEntity<ErrorResponse> payment(PaymentException ex, WebRequest req) {
        log.error("Payment error", ex);
        return build(HttpStatus.INTERNAL_SERVER_ERROR, "Payment Error", ex.getMessage(), req, null);
    }
